...
// Northern Norway: reconnect the sampled Nordkinn headland to the mainland.
rows[6][72]=terrainAt(G.hexToGeo(72,6));homes[6][72]='no';
// A sub-grid border sliver beside Lake Inari is not a Norwegian enclave.
rows[11][73]=terrainAt(G.hexToGeo(73,11));homes[11][73]='fi';
...
// Preserve Crete's narrow east-west spine, rather than four detached centre samples.
const creteSpine=[[23.7,35.4],[24.2,35.32],[24.8,35.2],[25.25,35.18],[25.75,35.12],[26.25,35.23]];
for(let i=1;i<creteSpine.length;i++)for(const [c,r]of lineHexes(creteSpine[i-1],creteSpine[i])){
 rows[r][c]=terrainAt(G.hexToGeo(c,r));homes[r][c]='gr';
}
// IJsselmeer was enclosed by the Afsluitdijk in 1932: inland lake, not open sea.
rows[57][44]='l';homes[57][44]=null;
// Explicit fjord axes retain real outlets otherwise lost to 45 km centre sampling.
const fjordAxes=[
 [[11.27,63.97],[11.05,63.8],[10.8,63.65],[10.2,63.6],[9.7,63.65],[9.4,63.7]],
 [[5.95,59.75],[5.65,59.7],[5.4,59.7],[5.15,59.65]],
];
for(const line of fjordAxes)for(let i=1;i<line.length;i++)for(const [c,r]of lineHexes(line[i-1],line[i])){
 rows[r][c]='~';homes[r][c]=null;
}
...
// Retain the existing city cell when restoring the surrounding island coastline.
anchors.heraklion='gr';anchorLocations.heraklion=[77,107];
...
const riverNames={Ural:'乌拉尔河',Rhine:'莱茵河',Danube:'多瑙河',Seine:'塞纳河',Loire:'卢瓦尔河',Rhone:'罗讷河',Po:'波河',Elbe:'易北河',Oder:'奥得河',Vistula:'维斯瓦河',Dnieper:'第聂伯河',Don:'顿河',Volga:'伏尔加河',Dniester:'德涅斯特河',Daugava:'西德维纳河',Dvina:'西德维纳河',Tagus:'塔霍河',Douro:'杜罗河',Ebro:'埃布罗河',Dnepr:'第聂伯河'};
...
const map={...G.spec,terrainRevision:2,rows:rows.map(r=>r.join('')),homes,cityLocations,deployments,rivers,canals,riverEdges:[...riverEdges],blockedEdges:[...new Set(blockedEdges)],landingCells,...labelData,land:[],shallows:[]};
