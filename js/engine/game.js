...
class Game {
  ...
  airBuildError(cityKey,eqKey,faction=this.playerFaction) {
    ...
    if(!['axis','west','sov'].includes(faction)||ci.owner!==faction)return T('只能在己方机场组建');
  ...
  rebaseAir(u,cityKey) {
    ...
    this.pushLog(F('{0} 转场至{1}机场，本回合行动结束。', u.eq.n, ci.n),'info');return true;
  }
  ...
  genericShipName(cls) { return {bc:T('战列巡洋舰'),cve:T('护航航母'),cv:T('航空母舰')}[cls]||CLASSES[cls].name; }
  ...
  constructionError(cityKey,kind,site=null,faction=this.playerFaction) {
    ...
    if(!['axis','west','sov'].includes(faction)||ci.owner!==faction)return T('只能在己方城市建设');
  ...
  navalBuildError(cityKey,eqKey,faction) {
    ...
    if(!['axis','west','sov'].includes(faction)||ci.owner!==faction)return T('只能在己方军港建造');
  ...
  captureCity(city, faction, byUnit) {
    ...
    this.pushLog(F('{0} 占领 {1} {2}！', this.factionName(faction), cn, city.n), 'war');
    // 首都易手 → 该国全境易帜
    if (city.cap) {
      let flipped = 0;
      ...
      this.terrDirty = true;
      this.pushLog(F('{0}首都 {1} 陷落，{0}全境沦陷！（{2} 座城市易手）', cn, city.n, flipped), 'war');
    }
  ...
  processEvents() {
    for (const ev of EVENTS) {
      ...
      if (ev.kind === 'log') { this.pushLog(F('【{0}】{1}', ev.title, ev.text), 'event'); this.pendingEvents.push(ev); }
