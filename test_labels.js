...
for(const [z,expected]of [[.025,0],[.23999,0],[.24,1],[.64999,1],[.65,2],[2.19999,2],[2.2,3]])assert.equal(L.tier(z,M),expected);
...
for(const n of ['克里木半岛','亚美尼亚','格鲁吉亚','阿塞拜疆','库班地区','巴库','塞瓦斯托波尔','罗斯托夫']){
 assert(regional.has(n),n+' intermediate level');assert(!overview.has(n)&&!local.has(n));
}
...
assert.equal(L.layout(crowded.map(c=>({...c,all:true})),view,measure).length,8,'maximum zoom never drops visible city names');
