...
function showStart(fac = 'axis', diff = 'normal') {
  ...
  const FINFO = {
    ...
    sov: { name: '苏联', color: '#8f1f16',
      desc: '对德战争前可备战，但1939年11月会按条件触发对芬兰的局部战争。战争爆发后以空间换时间，用钢铁洪流淹没侵略者，最终攻克柏林。适合喜欢大兵团作战的指挥官。',
  ...
  openModal(`
    <div class="modal" style="max-width:860px">
      ...
      选择你的阵营，改写 1939-1945 年的欧洲命运。占领敌方首都可触发投降；冬季战争按特别停战条款结算。</div>
  ...
  openModal(`
    <div class="modal help-modal" role="dialog" aria-modal="true" aria-labelledby="help-title">
      ...
      <div class="help-body">
        ...
        <h4>■ 回合与经济</h4>
        每回合=1个月，三个阵营开局各1200金。城市按面板数值产出金币。普通陆军在己方城市+25兵力／回合，己方领土+12，最高100；海运中的陆军不补员。T-34在己方城市／己方领土／敌对领土分别恢复35／20／10；核污染格停止补员。
        <h4>■ 城市招募与设施建设</h4>
        普通城市按民兵、徒步步兵、机动步兵分栏招募11类步兵；工厂单独组建5类炮兵与5类装甲。陆军只能出现在空闲己方城市格，新兵当回合不能行动，不会部署到邻格。非军事区不能招兵。<br>
        城市可建设工厂（150金／3回合）、机场（120金／2回合）；沿海城市可选择相邻合适海格建港（180金／3回合）。开工扣费，同类设施不能重复建设，不同设施可同时建设；当前不提供取消。工厂开放生产，不额外增加收入。已建和在建设施随城市易手；港口工地被非己方单位占据时暂停建设。<br>
        步兵／炮兵／装甲按1939、1941、1943三级解锁；生产当年最新型号，已造部队不会自动升级。主要国家有特色装备，具体效果见部队与生产面板。
        <h4>■ 战斗规则</h4>
        伤害 ≈ 42 × 攻/(攻+防)。防御方获得地形加成；兵力越低输出越低。<br>
        炮兵/空军无视地形防御加成；步兵/装甲在相邻格反击，海军可在自身射程和目标限制内反击，包括对来袭空军的防空还击。<br>
        移动后没有可攻击目标会自动待命。仅未造成战局影响的最后一次普通移动可撤销；占领、开战、投降、割让、吞并及事件均不能撤销。攻击、其它部队有效移动／攻击或结束回合后失效。招募、建设等改变战局的操作也会结束撤销机会。<br>
        尚未行动的陆海军即使移动力不足，也可移动到一个合法且空闲的相邻格，耗尽本回合移动；不可越过陆海限制、禁行边或单位阻挡。<br>
        进入敌军相邻格会被<b>控制区(ZOC)</b>截停（具有忽略控制区技能的将领与空军除外）。<br>
        <b>驻防</b>+30%防御，跨回合自动保持，未选中时显示为暗色，不列入“下一部队”与待命提示；手动移动或攻击后解除。己方部队面板可解散部队，不返还经济，将领回到待指派名单；载有伞兵的运输机需先卸载。老练度（击杀获取经验）最多+24%攻防。
        <h4>■ 兵种用途与目标选择</h4>
        ...
        <h4>■ 将领与成长</h4>
        116位史实将领，开局92位已部署、24位待指派。通过部队面板“将领”指派同阵营可用将领，海军暂不支持。每击杀3个单位升一星，最高五星，每升一星额外+4%攻防。普通技能仅影响所指挥单位，带兵种限定时需匹配；光环影响相邻友军，多个光环取最强，不影响自身或同格单位。
        <h4>■ 地形防御加成</h4>
        ...
        <h4>■ 机场、伞降与核打击</h4>
        点击城市“打开机场·组建空军”管理驻机。7类飞机在机场内可与陆军同格；战斗机主要攻击机场敌机，CAS／战术轰炸机对地，海军轰炸机反舰。运输机不能攻击；旧飞机不自动升级。轻战／CAS／海轰低价，重战／战术轰炸／运输机中价，战略轰炸机最高；后两级机型价格比基础高10%／20%。<br>
        伞兵在城市招募，移动到有己方运输机的机场后，双方未行动即可装载；一机一支伞兵。伞降目标须在航程内且为合法空闲陆格，飞机留在原机场，双方当回合行动结束；运输机被毁会连带损失载员。机场失守会损失仍驻在该机场的敌机，条约割让则执行撤军。<br>
        1945年起仅战略轰炸机可核打击，每次2000金，中心格全部单位消灭、邻格损失75兵力，敌我均受影响。中心与邻格污染6回合，每回合损失20兵力、停止补员，污染城市收入为0。核打击不自动占城，波及中立或非交战国家可引发战争，执行前需确认。
        <h4>■ 军港与海军</h4>
        点击海格 ⚓ 或城市面板的“打开军港”，建造当前年份最先进舰型；泊位必须空闲且经济充足，没有额外造舰数量或舰队规模上限。新舰下回合行动，已造旧舰不自动升级。计划舰和游戏解锁年份会在面板中说明。<br>
        ...
        等移动消耗的路线优先避开中立国。会引发战争的移动、攻击或伞降须先确认，弹窗列明受影响国家及参战后果；取消不消耗行动。中立国可进攻。巴巴罗萨行动前，苏联进攻中立国仅触发双方局部战争，不牵连同盟国；其他情况沿用倒向敌方阵营的规则。<br>占领城市会转移其附属领土；等距属于多座城市的格子，只有全部相关城市归同一占领国才转移。割让会迁走原国家部队，吞并则解散原国家全部部队。政区颜色与边界随实际控制国变化。
        <h4>■ 教程、存档与主菜单</h4>
        阵营选择页提供8×5格的新手演习，教程不覆盖正式存档。顶栏“主菜单”保存战役并返回阵营选择，行动结算时需等待结束。存档保存在当前浏览器，在线与离线地址不共用，清理浏览器数据可能丢失存档；重要进度请点击“存档”。新开局采用完整初始部署，旧存档不追补已过去的事件。<br>
        <a href="README.md" target="_blank" rel="noopener">完整游戏说明</a> · <a href="DIPLOMACY.md" target="_blank" rel="noopener">外交与条约细则</a>
        <h4>■ 音乐与署名</h4>
        ...
        <h4>■ 历史事件</h4>
        波兰被德国迫降后按条件触发苏德瓜分；1939.11苏芬冬季战争；1940.6波罗的海最后通牒、比萨拉比亚割让及意大利参战；1940.11匈罗入轴；1941.6巴巴罗萨；1941.12美国参战；1944.6诺曼底登陆。事件可能因国家已被吞并、外交状态或无可用部署格而不发生，部分战报仅叙事。<br>
        冬季战争：芬兰夺取列宁格勒迫使苏联停战；苏联夺取赫尔辛基后芬兰割让维堡及卡累利阿附属地区，但保留独立。<br>
        从1941年起每年12—2月，位于苏联法律领土且该格由轴心国控制的轴心单位每回合-12兵力。战役不在1945年自动结束。
      </div>
    </div>`);
