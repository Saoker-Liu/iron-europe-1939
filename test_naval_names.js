...
assert.equal(navy.length,91);assert.equal(start.units.length,D.INITIAL_UNITS.length+91);
assert.equal(new Set(start.units.map(u=>key(u.c,u.r))).size,start.units.length);
