...
- 固定海运航线已删除，改为陆军购买运输舰艇后逐格航行。下海与上岸分别耗尽行动，海洋与湖泊区分处理；舰艇科技和数值属于游戏抽象，详见README。
- 飞机可越过海洋和湖泊，但回合结束须停在陆地；主要河流的跨河移动额外消耗1点。
- 1939归属仅为开局底图；战役中的割让、吞并和投降改变国家领土。占领按本国最近城市的附属领土改变控制权，等距格须全部相关城市归同一占领国才转移。政区颜色与边界随当前控制国重绘。仍不是完整的补给线或逐格军事占领模拟，事件领土使用45公里格网概化，详见[外交与领土规则](DIPLOMACY.md)。
...
地区文字按全览、次级、局部三个层级替换；城市名称逐级累加，倍率1.3起显示当前视野内全部城市名，距2.2最大倍率尚有约4～5次滚轮。城市名优先排版且不省略，碰撞时避让或省略地区文字。名称层级是为地图阅读服务的设计，不是对1939行政区划级别的严格编码。苏格兰、威尔士、北爱尔兰与阿布哈兹等沿用地区名称，未推定独立国家地位。
...
安纳托利亚原地区层标签改为全览层“安纳托利亚半岛”，避免同一地理单元跨层重复。地区名称遵循层级替换和碰撞避让，城市名称随缩放累加保留；倍率1.3起仅显示全部城市名称。地理术语参考[国家地理学会：欧洲自然地理](https://education.nationalgeographic.org/resource/europe-physical-geography/)，国界沿用本项目的1939快照。

## 当前海运规则取代历史版本航线

本文件v4—v8地图沿革中提到的固定海运／渡运连接只记录当时实现，现已全部删除。岛屿可达性改为通过相邻海格航行，海峡阻断边仍禁止陆军步行横穿。国界、陆海格位和城市未因运输机制改变；基尔运河仍是窄水道地理标识，不自动成为海洋格。

三档科技是二战登陆运输能力的游戏概化：普通运输船、1942年两栖运输舰、1944年两栖突击舰（运输舰配合突击艇的编队，不是现代直升机两栖舰）。美国海军历史资料记录LST-383于1942年10月服役、1944年诺曼底作战使用LCT等登陆装备；这些资料支持时代背景，不意味着本游戏的固定数值、全阵营统一解锁和任意海岸下船为历史实况。
...
克里特岛的狭长轮廓在45公里采样下被海格切断；现沿岛屿中轴补足陆格，保持整个岛屿可步行连通，并保留与希腊大陆之间的海洋。伊拉克利翁城市格不移动。挪威贝特斯塔峡湾（特隆赫姆峡湾内湾）和奥克拉峡湾的出口在采样时封闭，现用连续海格概化狭窄航道；这不表示真实峡湾宽达45公里。

荷兰(44,57)对应艾瑟尔湖。阿夫鲁戴克大堤于1932年将其与瓦登海分开，因此1939年应表达为湖泊，而非强行挖开通海航道。本次将该格从海洋改为湖泊，陆军、海军均不可进入。
